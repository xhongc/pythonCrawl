...
class AdminUserSerializer(serializers.ModelSerializer):
    ...
    is_active = serializers.IntegerField(label='启用状态', min_value=0,max_value=1,required=False,help_text='0关闭，1启用')
    login_ip = serializers.CharField(read_only=True)
    last_login_time = serializers.CharField(read_only=True)

    class Meta:
        model = UserAdmin
        fields = ('id', 'username', 'password', 'display_password','url', 'is_active', 'ymt_name', 'ymt_pwd', 'login_ip', 'last_login_time')
