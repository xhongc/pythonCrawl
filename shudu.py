...
class solution(object):
    ...
    def check(self, x, y, value):  # 检查每行每列及每宫是否有相同项
        ...
        for row_all in self.b:
            if row_all[y] == value:
                return False
        row, col = x // 3 * 3, y // 2 * 2
        row3col3 = self.b[row][col:col + 3] + self.b[row + 1][col:col + 3]
        print(x,y,row,col,row3col3)
    ...
    def get_next(self, x, y):  # 得到下一个未填项
        for next_soulu in range(y + 1, 6):
            if self.b[x][next_soulu] == 0:
                return x, next_soulu
        for row_n in range(x + 1, 6):
