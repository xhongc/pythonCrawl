...
from utils.jiupaipay import jiupaipay
...
class OrderSerializer(serializers.ModelSerializer):
    ...
    pay_type = serializers.CharField(required=False)
    post_script = serializers.CharField(required=False, read_only=True)
    # 支付订单的url
    alipay_url = serializers.SerializerMethodField(read_only=True)

    def get_alipay_url(self, obj):

        if obj.pay_type == 'alipay':
            alipay = AliPay(
                appid="2016091500517456",
                app_notify_url="http://47.93.198.159:8000/alipay/return/",
                app_private_key_path=private_key_path,
            ...
            return re_url
        elif obj.pay_type == 'jiupaipay':
            moneny = obj.order_mount
            if moneny:
                moneny = int(moneny * 100)
                print(moneny)
                re_url = jiupaipay(amount=moneny)
                return re_url
            else:
                return None
        else:
